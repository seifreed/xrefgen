...
DEFAULT_REGISTRY: Tuple[ModuleSpec, ...] = (
    ModuleSpec(
        "data_flow", "modules.infrastructure.ida.analysis.data_flow", "DataFlowAnalyzer"
    ),
    ModuleSpec(
        "obfuscation",
        "modules.infrastructure.ida.obfuscation.detector",
        "ObfuscationDetector",
    ),
    ModuleSpec(
        "architecture",
        "modules.infrastructure.ida.architecture.cross_arch",
        "CrossArchAnalyzer",
    ),
...
def build_modules(
    config: Dict[str, Any], registry: Tuple[ModuleSpec, ...] = DEFAULT_REGISTRY
):
    ...
    for spec in registry:
        enabled = bool(
            config.get("modules", {}).get(spec.key, {}).get("enabled", False)
        )
        if not enabled:
            continue
        try:
